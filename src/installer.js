...
let EXTERNAL = /^\w[a-z\-0-9\.]+$/; // Match "react", "path", "fs", "lodash.random", etc.
let PEERS = /UNMET PEER DEPENDENCY ([a-z\-0-9\.]+)@(.+)/gm;
...
module.exports.check = function check(request) {
    ...
    let namespaced = request.charAt(0) === "@";
    let dep = request.split("/")
...
module.exports.checkBabel = function checkBabel(params) {
    if (params.babel) {
        var babelOpts = params.babel;
    } else {
        try {
